...
class search:
    def __init__(self):
        ...
        self.FileHandlingObj.ReadFile("artists.dat")
...
# create token from the string description and remove stop word(common word)
    ...
    def Search(self,query):
        ...
        for result_index in range(5):
            artistName.append(self.artistData.loc[self.sorted_similarity[result_index],'title'])
            artistDes.append(self.artistData.loc[self.sorted_similarity[result_index],'overview'])
        retrive_data.update({"Artist":artist_name})
        retrive_data.update({"Description": artist_description})
        return  retrive_data
